...
class UpdateRafflePeriodRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'code' => [
                ...
                'string',
                'max:50',
                Rule::unique('raffle_periods', 'code')->ignore($this->route('raffle_period')),
            ],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            $period = $this->route('raffle_period');

            if ($period && $this->input('status') === 'active' && $period->drawing_status === 'completed') {
                $validator->errors()->add('status', 'Periode yang pengundiannya sudah selesai tidak dapat diaktifkan kembali.');
            }
        });
    }
