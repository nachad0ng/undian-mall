                            ...
                            <div class="col">
                                <form method="GET" action="{{ route('admin.permissions.index') }}" class="d-flex gap-2">
                                    <input type="text" name="search" class="form-control d-inline-block"
                                        style="max-width: 200px;" placeholder="Search permission..."
                                        value="{{ $search }}">
                                            ...
                                            @if ($permission->roles->count() > 0)
                                                <div>
                                                    @foreach ($permission->roles->take(2) as $role)
                                                        <span class="badge bg-green-lt">{{ $role->name }}</span>
                                                    @endforeach
                                                    @if ($permission->roles->count() > 2)
                                                        <span
                                                            class="badge bg-gray-lt">+{{ $permission->roles->count() - 2 }}
                                                            more</span>
                                                    @endif
                                                </div>
                                            @else
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="{{ route('admin.permissions.edit', $permission) }}"
                                                    class="btn btn-icon btn-ghost-primary" title="Edit">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center text-secondary py-4">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon mb-2" viewBox="0 0 24 24"
                                                stroke-width="2" stroke="currentColor" fill="none"
                                                stroke-linecap="round" stroke-linejoin="round"
                                                style="display: inline-block; width: 48px; height: 48px; opacity: 0.5;">
                                                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                <path
                                                    d="M19 6.364l1.636 -1.636a2 2 0 0 1 2.828 2.828l-.001 .001l-1.636 1.636m-2.182 -2.182a2 2 0 0 1 2.182 2.182v4m-6 2a2 2 0 0 1 -2 -2v-4a2 2 0 0 1 2 -2h4a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-4" />
